                ...
                <div class="glass-card group p-8 rounded-[2.5rem] hover:border-xbox-green/40 transition-all duration-500 overflow-hidden relative">
                    ...
                    <div class="flex items-center gap-6 relative z-10">
                        <div class="w-24 h-24 rounded-[1.5rem] overflow-hidden flex-shrink-0 shadow-2xl border-2 border-white/5 group-hover:border-xbox-green/50 transition-all duration-500">
                            <img src="<?= $title['imageUri'] ?? 'img/default_game.jpg' ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="Game">
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <span class="text-lg font-black text-white"><?= number_format($title['achievement']['currentGamerscore'] ?? 0, 0, ',', '.') ?> <span class="text-gray-600 text-xs">/ <?= number_format($title['achievement']['totalGamerscore'] ?? 1000, 0, ',', '.') ?></span></span>
                            </div>
                            <!-- Progress Bar -->
                            ...
                            <div class="flex justify-between mt-2">
                                <span class="text-[9px] font-black text-secondary uppercase tracking-widest"><?= $title['achievement']['progressPercentage'] ?? 0 ?>% Completo</span>
                                <span class="text-[9px] font-black text-secondary uppercase tracking-widest"><?= $title['achievement']['currentAchievements'] ?? 0 ?> Troféus</span>
                            </div>
                        </div>
                    </div>
                    <!-- Action Overlay -->
                    <a href="jogo?id=<?= $title['titleId'] ?>" class="absolute inset-0 z-20"></a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
