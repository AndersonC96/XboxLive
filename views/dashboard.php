        ...
        <div class="lg:col-span-1">
            <div class="glass-card rounded-3xl overflow-hidden p-8 flex flex-col items-center text-center">
                <div class="relative mb-6">
                    <img src="<?= $userProfile['gamerpic']; ?>" alt="Avatar" class="w-28 h-28 rounded-3xl border-4 border-xbox-green shadow-2xl">
                ...
                <h2 class="text-2xl font-bold mb-1"><?= htmlspecialchars($userProfile['gamertag']); ?></h2>
                <div class="flex items-center gap-2 mb-8">
                    <img src="img/gs.png" class="w-4 h-4" alt="GS">
                    <span class="text-xbox-green font-bold"><?= htmlspecialchars($userProfile['gamerscore']); ?></span>
                </div>
                ...
                <div class="w-full space-y-4 text-left">
                    ...
                    <div class="p-4 bg-white/5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-gray-400 leading-relaxed italic">"<?= htmlspecialchars($userProfile['bio']); ?>"</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="glass-card rounded-3xl p-8">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <span class="px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest <?= ($presenceState === 'Online') ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'; ?>">
            ...
            <div>
                <div class="flex items-center justify-between mb-6 px-2">
                    ...
                    <a href="historico" class="text-xs font-bold text-gray-500 hover:text-xbox-green transition-colors uppercase tracking-widest">Ver tudo</a>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php if (!empty($recentTitles)): ?>
                        <?php foreach ($recentTitles as $title): ?>
                            <a href="jogo?id=<?= $title['titleId']; ?>" class="glass-card group p-4 rounded-2xl flex items-center gap-4 hover:border-xbox-green/40 transition-all">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-bold text-white truncate mb-1"><?= htmlspecialchars($title['name']); ?></h4>
                                    <div class="flex items-center gap-2">
                                        <div class="h-1.5 flex-1 bg-white/5 rounded-full overflow-hidden">
                                            <div class="h-full bg-xbox-green w-[<?= $title['achievement']['progressPercentage'] ?? 0; ?>%]"></div>
                                        </div>
                                        <span class="text-[10px] font-bold text-gray-500"><?= $title['achievement']['progressPercentage'] ?? 0; ?>%</span>
                                    </div>
                                </div>
