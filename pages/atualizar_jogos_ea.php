<?php
    ...
    function adicionarJogosNoBanco() {
        global $pdo;
        $response = openXBLRequest('gamepass/ea-play');
        if (isset($response) && is_array($response)) {
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM ea_gamepass WHERE game_id = :game_id");
            $insertStmt = $pdo->prepare("INSERT INTO ea_gamepass (game_id) VALUES (:game_id)");
