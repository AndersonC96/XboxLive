...
$stmt = $pdo->prepare("SELECT game_id FROM ea_gamepass");
$stmt->execute();
...
<main class="xbox-content">
    <div class="xbox-page space-y-6">
        ...
        <?php if (!empty($response['Products'])) : ?>
            <div id="eaList" class="friend-grid single-column">
...
<script>
    ...
    const xboxContentEA = document.querySelector('.xbox-content');
    const defaultBgEA = xboxContentEA ? window.getComputedStyle(xboxContentEA).backgroundImage : '';
    ...
    if (eaList) {
        ...
        eaCards.forEach(card => {
            card.addEventListener('mouseenter', () => {
                ...
                if (hero && xboxContentEA) {
                    xboxContentEA.style.backgroundImage = `linear-gradient(to bottom, rgba(0,0,0,0.65), rgba(0,20,10,0.9)), url('${hero}')`;
                    xboxContentEA.style.backgroundSize = 'cover';
                    xboxContentEA.style.backgroundPosition = 'center';
                    xboxContentEA.style.transition = 'background-image 0.45s ease';
                }
            });
            card.addEventListener('mouseleave', () => {
                if (xboxContentEA) xboxContentEA.style.backgroundImage = defaultBgEA;
            });
        });
    }
</script>
