<?php
    ...
    function adicionarJogosNoBanco() {
        global $pdo;
        $response = openXBLRequest('gamepass/all');
        if (isset($response) && is_array($response)) {
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM gamepass_games WHERE game_id = :game_id");
            $insertStmt = $pdo->prepare("INSERT INTO gamepass_games (game_id) VALUES (:game_id)");
