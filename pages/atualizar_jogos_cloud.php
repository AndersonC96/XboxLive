<?php
    ...
    function adicionarJogosNoBanco() {
        global $pdo;
        $response = openXBLRequest('gamepass/no-controller');
        if (isset($response) && is_array($response)) {
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM cloud_gamepass WHERE game_id = :game_id");
            $insertStmt = $pdo->prepare("INSERT INTO cloud_gamepass (game_id) VALUES (:game_id)");
